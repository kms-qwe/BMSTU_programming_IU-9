...
// @Description Builds a PDF employees report for the selected period.
// @Tags report
// @Accept json
...
// @Router /api/report/employees/pdf [post]
func (a *API) createEmployeesReport(ctx *gin.Context) error {
	...
	return common.PDF(ctx, fmt.Sprintf("employees-report-%s-%s.pdf", filter.From.Format("2006-01-02"), filter.To.Format("2006-01-02")), content)
}
